...
Class Project extends CI_Controller
{
	...
	public function project_list()
	{
		...
		if($this->form_validation->run() == FALSE)
		{
			...
			$query = $this->Project_detail->project_result($data);
			...
			foreach ($material_data as $key1 => $value2) {
					...
					$material_data[$key1]['material_name'] = $material_data[$key1]['materials_name'];
					$material_data[$key1]['material_amount'] = $material_data[$key1]['amount'];
					$material_data[$key1]['consumption'] = $final_consumption[$key1]['consumption'];
					unset($material_data[$key1]["id"]);
					unset($material_data[$key1]["materials_name"]);
					unset($material_data[$key1]["amount"]);
