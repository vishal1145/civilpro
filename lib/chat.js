...
app.controller("mainController", function ($scope, $http, $rootScope) {
    ...
    if ($scope.userId) {
        $http.get(window.ipAddress + ':9100/GETUSERCHATGROUPS/' + $scope.userId).then((res) => {
    ...
    function startChat() {
        window.socket = io(window.ipAddress + ":9200");
...
app.controller("chatController", function ($scope, $http, $rootScope) {
    ...
    if (userId) {
        $http.get(window.ipAddress + ':9100/GETUSERCHATGROUPS/' + userId).then((res) => {
            localStorage.setItem("GROUPS", JSON.stringify(res.data.groups));
            localStorage.setItem("MESSAGES", JSON.stringify(res.data.messages));
            $rootScope.setUnreadCount();
            $scope.loadGroups();
