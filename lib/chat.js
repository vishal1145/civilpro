    ...
    if ($scope.userId) {
        $http.get(window.ipAddress + ':8100/GETUSERCHATGROUPS/' + $scope.userId).then((res) => {
            ...
            for (var i = 0; i < res.data.messages.length; i++) {
                for (var j = 0; j < res.data.messages[i].ReadBy.length; j++) {
                    if (res.data.messages[i].ReadBy[j].userid !== $scope.userId) {
                        allUnreadMessages.push(res.data.messages[i]);
                    }
                }
            }

            $rootScope.unreadCount = (allUnreadMessages || []).length;
...
app.controller("chatController", function ($scope, $http, $rootScope) {
            ...
            if ($scope.messages[i].GroupId == group.id) {
                for (var j = 0; j < $scope.messages[i].ReadBy.length; j++) {
                    if ($scope.messages[i].ReadBy[j].userid !== $scope.userId) {
                        allUnreadMessages.push($scope.messages[i]);
                    }
                }
            }
        }
    ...
    $scope.openGroup = function (group) {
        ...
        //set files data 
        //step 1- find all the message with group id and ismedia = = true 
        //step 2 - display my files 
    }
    ...
    setInterval(function () {
        if ($scope.messages && $scope.messages.length) {
            for (var mcounter = 0; mcounter < $scope.messages.length; mcounter++) {
                //timer($this.getAttribute('title') || $this.getAttribute('datetime'));
